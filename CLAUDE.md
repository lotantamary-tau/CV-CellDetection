...
| **DL-seeding (CellPose→CNMF) hybrid** | A pipeline that replaces CNMF's brightness seeding with **CellPose-SAM** cell detection, then extracts traces via seeded CNMF (footprints frozen). Beats the greedy baseline on coverage + merges. See [cnmf_toolkit/DL_SEEDING_WORKFLOW.md](cnmf_toolkit/DL_SEEDING_WORKFLOW.md) for the full end-to-end steps. |
| [cnmf_toolkit/DL_SEEDING_WORKFLOW.md](cnmf_toolkit/DL_SEEDING_WORKFLOW.md) | Step-by-step teammate guide for the CellPose→CNMF pipeline (which files to edit, the Colab↔local transfer, the toggles). |
| [cnmf_toolkit/cellpose_export_projection.py](cnmf_toolkit/cellpose_export_projection.py) | Build mean/max/correlation projections of a movie → the 2D image you upload to Colab for CellPose. |
| [cnmf_toolkit/cellpose_mask_to_A.py](cnmf_toolkit/cellpose_mask_to_A.py) | Convert a CellPose label mask → CNMF-style `_A.npz` so `ground_truth_scorer.py` can score a segmentation directly. |
| [notebooks/cellpose_colab.ipynb](notebooks/cellpose_colab.ipynb) | Colab (GPU) notebook: runs CellPose-SAM (`cpsam_v2`) on a projection → downloads a cell mask. Step 1 of the hybrid. |
| `notebooks/OPCal_cell detection_caiman_150226_dl-seeding.ipynb` | The local hybrid notebook: CellPose mask → seeded CNMF (intensity-weighted seeds, min-junk-filter, frozen footprints) → traces. Step 2 of the hybrid. CONFIG cell at the top sets the 3 paths. |
